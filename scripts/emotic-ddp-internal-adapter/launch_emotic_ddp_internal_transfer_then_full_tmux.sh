...
SESSION="ddp_internal_next"

for path in \
  "${ROOT}/output/emotic_b5c3_ddp_semantic_tau2/checkpoints/task0.pth" \
  "${ROOT}/output/emotic_ddp_internal_adapter_16shot_seed0/task0_val_path_features.pt" \
...
tmux new-session -d -s "${SESSION}" \
  "cd '${ROOT}' && GPU=0 bash scripts/emotic-ddp-internal-adapter/run_emotic_ddp_internal_transfer_then_full.sh"
