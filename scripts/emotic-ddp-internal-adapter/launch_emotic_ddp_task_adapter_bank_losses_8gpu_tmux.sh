...
for index in "${!GPUS[@]}"; do
  ...
  command="cd '${ROOT}' && GPU='${gpu}' LANE_FILE='${lane_file}' bash scripts/emotic-ddp-internal-adapter/run_emotic_ddp_task_adapter_bank_loss_lane.sh 2>&1 | tee '${STATE_DIR}/${window}.log'; code=\${PIPESTATUS[0]}; echo LANE_EXIT_CODE=\${code}; exec bash"
  if (( index == 0 )); then
...
tmux new-window -t "${SESSION}" -n summary \
  "cd '${ROOT}' && MODES='${MODES}' LOSSES='${LOSSES}' SEEDS='${SEEDS}' SUMMARY_OUTPUT_DIR='${SUMMARY_OUTPUT_DIR}' bash scripts/emotic-ddp-internal-adapter/wait_and_summarize_emotic_ddp_task_adapter_bank_losses.sh 2>&1 | tee '${STATE_DIR}/orchestrator.log'; code=\${PIPESTATUS[0]}; echo SUMMARY_EXIT_CODE=\${code}; exec bash"
