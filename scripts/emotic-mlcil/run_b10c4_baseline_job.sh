...
[[ "$(git rev-parse HEAD)" == "${EXPECTED_GIT_COMMIT}" ]] || {
  echo "B10-C4 job is not running the frozen sweep commit" >&2
  exit 2
}
...
case "${METHOD}" in
  ...
  krt)
    ...
    ;;
  *)
    echo "Unsupported B10-C4 baseline: ${METHOD}" >&2
    exit 2
    ;;
esac

RESULT_ROOT="${RUN_OUTPUT_ROOT}/benchmarks/emotic_b10c4_v0.1/A/${DISPLAY_METHOD}/seed${SEED}"
