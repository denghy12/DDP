#!/usr/bin/env python3
"""One-step CUDA memory smoke for native EMOT-Net-FT."""
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the EMOT-Net memory smoke")
    payload = torch.load(args.native_init, map_location="cpu")
    model = EMOTNetFTModel(fusion_dim=256, dropout=0.5)
    ...
    loss = weighted_sigmoid_mse(model.current_logits(images), targets, weights)
    loss.backward()
    optimizer.step()
