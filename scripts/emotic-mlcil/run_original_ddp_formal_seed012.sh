...
[[ "${CONFIGURATION_LOCKED_CONFIRMATION}" == "ORIGINAL_DDP_TAU2_TRACK_A_V0_1" ]] || {
  echo "Invalid Original-DDP-Tau2 configuration-lock confirmation" >&2
  exit 2
}
[[ "$(git rev-parse HEAD)" == "${EXPECTED_GIT_COMMIT}" ]] || {
  echo "Original-DDP-Tau2 formal worker is not running the frozen commit" >&2
  exit 2
}
[[ -z "$(git status --porcelain)" ]] || {
  echo "Original-DDP-Tau2 formal worker requires a clean Git worktree" >&2
  exit 2
}
...
run_seed() {
  ...
  echo "Starting locked Original-DDP-Tau2 seed ${seed} on physical GPU ${gpu}"
  set +e
  RUN_ID="${RUN_ID}" \
  SEED="${seed}" \
  GPU="${gpu}" \
  ...
  CONFIGURATION_LOCKED_CONFIRMATION="${CONFIGURATION_LOCKED_CONFIRMATION}" \
    bash "${SCRIPT_DIR}/run_original_ddp_baseline.sh" >"${log_path}" 2>&1
  code=$?
  set -e

  if [[ "${code}" -eq 0 ]]; then
    printf 'seed=%s\nphysical_gpu=%s\nphase=three_seed_parallel\nexit_code=0\nlog=%s\n' \
      "${seed}" "${gpu}" "${log_path}" >"${marker_prefix}.done"
    echo "Completed locked Original-DDP-Tau2 seed ${seed} on GPU ${gpu}"
    return 0
  fi
  printf 'seed=%s\nphysical_gpu=%s\nphase=three_seed_parallel\nexit_code=%s\nlog=%s\n' \
    "${seed}" "${gpu}" "${code}" "${log_path}" >"${marker_prefix}.failed"
  echo "Original-DDP-Tau2 seed ${seed} failed with exit code ${code}; see ${log_path}" >&2
  return "${code}"
}
...
if [[ "${overall_code}" -ne 0 ]]; then
  echo "At least one Original-DDP-Tau2 seed failed; aggregation and packaging are blocked" >&2
  exit 1
fi
for seed in 0 1 2; do
  [[ -f "${STATE_DIR}/seed${seed}.done" ]] || {
    echo "Missing completion marker for Original-DDP-Tau2 seed ${seed}" >&2
    exit 1
  }
done

"${PYTHON}" "${SCRIPT_DIR}/validate_original_ddp_formal_results.py" \
...
echo "All three locked Original-DDP-Tau2 seeds completed and validated"
