...
OUTPUT_ROOT="${OUTPUT_ROOT:-/mnt/haoyuan/workspace/emotic_benchmark_runs/multi_lane_track_a_v0.1/${RUN_ID}}"
...
if [[ "${REPORTING_SPLIT}" == "test" ]]; then
  [[ "${CONFIGURATION_LOCKED_CONFIRMATION}" == "MULTI_LANE_TRACK_A_V0_1" ]] || {
    echo "Held-out MULTI-LANE test requires frozen configuration confirmation" >&2
    exit 2
  }
