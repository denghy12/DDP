#!/usr/bin/env python3
"""Validate and summarize all 36 locked EMOTIC B4-C2 result bundles."""
...
def _write_markdown(path: Path, methods: Sequence[Mapping[str, Any]]) -> None:
    lines = [
        "# EMOTIC B4-C2 Track-A 12-Baseline Summary",
        "",
    ...
    lines.extend(
        [
            ...
            "All method hyperparameters were transferred from the frozen B5-C3 runs without B4-C2 test tuning.",
        ]
    )
...
def main() -> None:
    ...
    for method_key, display_name, memory_display in METHODS:
        ...
        for seed in SEEDS:
            ...
            if manifest.get("protocol_id") != "emotic_b4c2_v0.1":
            ...
            if not isinstance(tasks, list) or [len(task) for task in tasks] != [4] + [2] * 11:
                raise ValueError(f"Not an alphabetic B4-C2 task split: {root}")
            ...
            if not isinstance(task_rows, list) or len(task_rows) != 12:
                raise ValueError(f"Expected twelve task metrics: {root}")
            if [int(row.get("task_id", -1)) for row in task_rows] != list(range(12)):
                raise ValueError(f"Task metric IDs differ: {root}")
            scores = sorted(
                (root / "scores").glob("task*_scores.pt"),
                key=lambda path: int(path.name[4 : path.name.index("_scores.pt")]),
            )
            if [path.name for path in scores] != [
                f"task{task}_scores.pt" for task in range(12)
            ]:
    ...
    payload = {
        ...
        "track": "A",
        "task_sizes": [4] + [2] * 11,
        "seen_class_counts": list(range(4, 27, 2)),
        ...
        "configuration_policy": "reuse_frozen_b5c3_hyperparameters_without_b4c2_test_tuning",
