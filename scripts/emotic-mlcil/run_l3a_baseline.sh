...
OUTPUT_ROOT="${OUTPUT_ROOT:-/mnt/haoyuan/workspace/emotic_benchmark_runs/l3a_track_a_v0.1/${RUN_ID}}"
...
if [[ "${REPORTING_SPLIT}" == "test" ]]; then
  [[ "${CONFIGURATION_LOCKED_CONFIRMATION}" == "L3A_TRACK_A_V0_1" ]] || {
    echo "Held-out L3A test requires frozen configuration confirmation" >&2
    exit 2
  }
