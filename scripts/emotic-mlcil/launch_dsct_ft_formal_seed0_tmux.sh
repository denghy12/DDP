...
[[ "${CONFIGURATION_LOCKED_CONFIRMATION}" == "DSCT_FT_TRACK_B_V0_1" ]] || { echo "Invalid DSCT-FT configuration lock" >&2; exit 2; }
...
[[ "${FREE_GPU_MIB}" =~ ^[0-9]+$ && "${FREE_GPU_MIB}" -ge "${MIN_FREE_GPU_MIB}" ]] || { echo "GPU ${GPU} has ${FREE_GPU_MIB:-unknown} MiB free; ${MIN_FREE_GPU_MIB} required" >&2; exit 2; }
...
PY
  ...
  CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" "${SCRIPT_DIR}/smoke_dsct_ft_training.py" \
    --source-root "${SOURCE_ROOT}" --pretrained-weights "${PRETRAINED}" --batch-size 4
...
echo "Run ID: ${RUN_ID}"
echo "GPU: ${GPU}; free memory: ${FREE_GPU_MIB} MiB"
