...
launch_job() {
  ...
  local command="cd '${ROOT}' && set -o pipefail; PROTOCOL_KEY='${protocol}' SEED='${seed}' GPU='${gpu}' RUN_ID='${RUN_ID}' RUN_ROOT='${RUN_ROOT}' bash scripts/emotic-mlcil/run_task_adapter_bank_multiprotocol_job.sh 2>&1 | tee '${LOG_DIR}/${name}.log'; code=\${PIPESTATUS[0]}; if (( code == 0 )); then touch '${STATE_DIR}/${name}.done'; else echo \${code} > '${STATE_DIR}/${name}.failed'; fi; echo JOB_EXIT_CODE=\${code}; exec bash"
  if (( index == 0 )); then
