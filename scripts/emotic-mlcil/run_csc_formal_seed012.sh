...
[[ "${CONFIGURATION_LOCKED_CONFIRMATION}" == "CSC_TRACK_A_V0_1" ]] || {
  echo "Invalid CSC configuration-lock confirmation" >&2
  exit 2
}
...
run_seed() {
  ...
  echo "Starting locked CSC seed ${seed} on physical GPU ${gpu}"
  set +e
  RUN_ID="${RUN_ID}" \
  SEED="${seed}" \
  GPU="${gpu}" \
  ...
  CONFIGURATION_LOCKED_CONFIRMATION="${CONFIGURATION_LOCKED_CONFIRMATION}" \
    bash "${SCRIPT_DIR}/run_csc_baseline.sh" >"${log_path}" 2>&1
  code=$?
  set -e
  ...
  if [[ "${code}" -eq 0 ]]; then
    ...
    echo "Completed locked CSC seed ${seed} on physical GPU ${gpu}"
    return 0
  fi
  printf 'seed=%s\nphysical_gpu=%s\nexit_code=%s\nlog=%s\n' \
    "${seed}" "${gpu}" "${code}" "${log_path}" >"${marker_prefix}.failed"
  echo "CSC seed ${seed} failed with exit code ${code}; see ${log_path}" >&2
  return "${code}"
}
...
if [[ "${overall_code}" -ne 0 ]]; then
  echo "At least one CSC formal seed failed; aggregation and packaging are blocked" >&2
  exit 1
fi
for seed in "${seed_values[@]}"; do
  [[ -f "${STATE_DIR}/seed${seed}.done" ]] || {
    echo "Missing completion marker for CSC seed ${seed}" >&2
    exit 1
  }
done

"${PYTHON}" "${SCRIPT_DIR}/validate_csc_formal_results.py" \
...
echo "All three locked CSC formal seeds completed and validated"
