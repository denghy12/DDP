...
SESSION="${SESSION:-emotic_b4c2_12baseline}"
RUN_ID="${RUN_ID:-b4c2_12baseline_seed012_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_BASE="${OUTPUT_BASE:-/mnt/haoyuan/workspace/emotic_benchmark_runs/b4c2_12baseline_v0.1}"
...
[[ -z "$(git status --porcelain)" ]] || {
  echo "B4-C2 formal sweep requires a clean Git worktree" >&2
  git status --short
  exit 2
}
...
[[ "${SLOTS_PER_GPU}" == "1" || "${SLOTS_PER_GPU}" == "2" ]] || {
  echo "SLOTS_PER_GPU must be 1 or 2" >&2
  exit 2
}
[[ "${GPU_MEMORY_BUDGET_MIB}" =~ ^[0-9]+$ ]] || {
  echo "GPU_MEMORY_BUDGET_MIB must be a positive integer" >&2
  exit 2
}
(( GPU_MEMORY_BUDGET_MIB >= 14000 && GPU_MEMORY_BUDGET_MIB <= 20000 )) || {
  echo "GPU_MEMORY_BUDGET_MIB must be in [14000, 20000] for guarded dual-slot scheduling" >&2
  exit 2
}
...
echo "Running B4-C2 protocol and regression preflight..."
{
...
assert protocol.protocol_id == "emotic_b4c2_v0.1"
assert [len(task) for task in protocol.tasks] == [4] + [2] * 11
...
assert standard.task_capacities == tuple(range(80, 521, 40))
...
PY
  ...
  echo "gpu_memory_budget_mib=${GPU_MEMORY_BUDGET_MIB}"
...
printf -v tmux_command \
  'cd %q && RUN_ID=%q RUN_OUTPUT_ROOT=%q GPUS=%q SLOTS_PER_GPU=%q GPU_MEMORY_BUDGET_MIB=%q PYTHON=%q DATA_ROOT=%q CLIP_MODEL_PATH=%q PROTOCOL=%q REPLAY_CONTRACT=%q DERPP_REPLAY_CONTRACT=%q AGCN_WORD_EMBEDDINGS=%q EXPECTED_GIT_COMMIT=%q CONFIGURATION_LOCKED_CONFIRMATION=%q bash %q 2>&1 | tee %q; code=${PIPESTATUS[0]}; echo B4C2_SWEEP_EXIT_CODE=$code; exec bash' \
  "${ROOT}" "${RUN_ID}" "${RUN_OUTPUT_ROOT}" "${GPUS}" "${SLOTS_PER_GPU}" "${GPU_MEMORY_BUDGET_MIB}" "${PYTHON}" \
  ...
  "${SCRIPT_DIR}/run_b4c2_12baseline_sweep.sh" "${LAUNCHER_LOG}"

tmux new-session -d -s "${SESSION}" "${tmux_command}"

echo "Started B4-C2 12-baseline sweep: ${SESSION}"
echo "Run ID: ${RUN_ID}"
echo "Commit: ${EXPECTED_GIT_COMMIT}"
echo "Scheduling: 36 jobs, eight GPUs, up to ${SLOTS_PER_GPU} processes per GPU, memory-guarded dynamic backfill"
echo "Per-GPU reservation budget: ${GPU_MEMORY_BUDGET_MIB} MiB; DER++ cannot share with ER/PRS"
